...
public class Server {
	...
	public static void main(String[] args) throws IOException {
		...
		while (true) {
			...
			System.out.println("\tRDF to N3 converter - " + AppConstants.APPCONVERTER);
			System.out.println("\tN3 Reader/Partitionner(PS) - " + AppConstants.APPREADERPS);
			System.out.println("\tPredicate Reader/Splitter(POS) - " + AppConstants.APPREADERPOS);
			System.out.println("\tCompressor for PS files(with optional "
					+ "Pre-Comparator for PS files) - "	+ AppConstants.APPCOMPRESS);
			...
			try {
				...
				if (userCmd == AppConstants.APPEXIT) {
					...
					result = Server.processAllFiles(userCmd);
	...
	static int pos(int programInd){
		...
		ExecutorService executor = Executors.newFixedThreadPool(myConfig._nbThreads);
        for (int i = 0; i < myConfig._nbThreads; i++) {
            Runnable thread = new ProcessingThread(String.valueOf(i), programInd, inputLists.get(i), 
            		posPath);
            executor.execute(thread);
        }
        executor.shutdown();
		...
		try{
			...
			switch(myConfig._compressMode){
				case AppConstants.APPCOMPRESS_INRAM : 
					...
					break;
				case AppConstants.APPCOMPRESS_MONET : 
					dbu = new MonetDBUtils();
					break;
				case AppConstants.APPCOMPRESS_MONGO : 
					dbu = new MongoDBUtils();
					break;
				case AppConstants.APPCOMPRESS_MYSQL : 
					dbu = new MySQLUtils();
					break;
				case AppConstants.APPCOMPRESS_ORACLE : 
					dbu = new OracleUtils();
					break;
				case AppConstants.APPCOMPRESS_POSTGRES : 
					dbu = new PostgreSQLUtils();
					break;
				case AppConstants.APPCOMPRESS_REDIS : 
					dbu = new RedisUtils();
					break;
				default : 
	...
	static int precompare(){
		...
		ExecutorService executor = Executors.newFixedThreadPool(myConfig._nbThreads);
        for (int i = 0; i < myConfig._nbThreads; i++) {
            Runnable thread = new ProcessingThread(String.valueOf(i), myConfig._precompareMode, inputLists.get(i), 
            		comparePath);
            executor.execute(thread);
        }
        executor.shutdown();
	...
	static int compare(){
		...
		LinkedList<LinkedList<FilePair>> inputLists = JobAssigner.assignJobs(toComparePairs, false);
		//Create and execute threads with assigned sub task
		ExecutorService executor = Executors.newFixedThreadPool(myConfig._nbThreads);
        for (int i = 0; i < myConfig._nbThreads; i++) {
            Runnable thread = new ProcessingThread(String.valueOf(i), myConfig._compareMode, inputLists.get(i), 
            		indicatorPath);
            executor.execute(thread);
        }
        executor.shutdown();
...
//		
//		//Create and execute threads with assigned sub task
//		ExecutorService executor = Executors.newFixedThreadPool(APPServer._nbThreads);
